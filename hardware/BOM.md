...
| # | Item | Qty | Search terms / notes |
|---|------|-----|----------------------|
| 2.7 | ESP32-S3 DevKit | 4 | `ESP32-S3 DevKitC N16R8` — start node, finish node, tree module, spare. Note GPIO26–37 are consumed by flash/PSRAM on `N16R8`; avoid them for capture inputs. The spare doubles as **D31**'s radio module: if `architecture.md` §11 #12 goes badly, the tree carries a second one holding Wi-Fi, the mapping and the master role as another device on the same bus. Nothing extra to buy for the bench — but the carrier PCB should lay the footprint, because before fabrication it costs nothing and afterwards it costs a revision |
